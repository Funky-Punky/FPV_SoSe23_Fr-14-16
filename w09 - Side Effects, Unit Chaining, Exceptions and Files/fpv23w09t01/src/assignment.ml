      ...
      | _ ->
          raise
            (My_Corrupt_database_file
               "wrong count of grade arguments")
  with
  | Failure _ ->
      raise (My_Corrupt_database_file "wrong grade format")
  | End_of_file ->
      raise
        (My_Corrupt_database_file "grade count was too large")
...
let read_student_from_chan chan =
  try
    ...
    | [
     first_name; last_name; id_str; semester_str; grade_count_str;
        ...
        if grade_count < 0 then
          raise
            (My_Corrupt_database_file
               "grade count can not be negative")
        else
    ...
    | _ ->
        raise
          (My_Corrupt_database_file
             "missing fields in studen header")
  with
  | End_of_file -> None
  | Failure _ ->
      raise
        (My_Corrupt_database_file
           "datatype error in student header")
